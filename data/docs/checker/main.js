...
const updateStatus = async (attempt_id, event, event_num, time, memory, comment) => {
  ...
  try {
    await execute("UPDATE attempts SET event = if(event_num = 2, event, ?), event_num = ?, time = GREATEST(time, COALESCE(?, 0)), memory = GREATEST(memory, COALESCE(?, 0)), comment = ? WHERE attempt_id = ?", [event, event_num, time, memory, comment, attempt_id]);
  } catch (err) {
  ...
  fs.readdir(temp_dir, (err, files) => {
    ...
    files
      ...
      .forEach((file) => {
        ...
        fs.unlink(file_path, (err) => (err ? console.error(`Failed to delete ${file_path}: ${err}`) : console.log(`Deleted: ${file_path}`)));
      });
  });
};
...
const runChecker = async (attempt_id, task_id, temp_dir, test_count, time_limit, memory_limit, script_compilation, script_run, image_name) => {
  const dockerArgs = ["run", "--rm", "-v", `${temp_dir}:/app/sandbox`, "--cpus", "2", "--memory", "2GB", "--memory-swap", "2GB", image_name, "bash", "-c", `./${image_name}.sh ${test_count} /app/sandbox '${script_compilation}' '${script_run}' ${time_limit}`];
...
const runMain = async (attempt_id, contest_id, task_id, lang_id, code, is_rerun = false) => {
  try {
    const [task, lang] = await Promise.all([execute(`SELECT * FROM tasks WHERE task_id = ?`, [task_id], 1), execute("SELECT * FROM lang WHERE group_id in (SELECT group_id FROM vw_tasks WHERE task_id = ? and contest_id = ?) AND lang_id = ?", [task_id, contest_id, lang_id], 1)]);
