...
const problemsSubmit = fnWrap(async (req, res) => {
  ...
  const [test_cases, lang, problem] = await Promise.all([fnGetTestCases(req.params.id), dbQueryOne("SELECT lang_code FROM languages WHERE lang_id = ?", [lang_id]), dbQueryOne("SELECT time_limit FROM problems WHERE problem_id = ?", [req.params.id])]);
