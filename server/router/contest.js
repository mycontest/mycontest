...
app.get("/code", [authStop], async (req, res) => {
      try {
            ...
            let code_path = path.join(__dirname, "../../compiler/tmp/" + parseInt(req.query.attempt_id) + "v/Main." + lang?.file_type);
            return res.render("pages/code", { data: req.data, pageInfo: "code", tasks, contest: req.data.contest, attempt_id: req.query.attempt_id, code: readCode(code_path) });
      } catch (err) {
            return res.redirect(`/contest/${req.data.contest_id}?error=${err.message}`);
      }
});
