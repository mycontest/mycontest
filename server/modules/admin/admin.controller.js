...
const adminProblemCreate = fnWrap(async (req, res) => {
  ...
  const result = await fnCreateProblem(
    { title, slug, difficulty, description: description || "", input_format: input_format || "", output_format: output_format || "", constraints: constraints || "", time_limit: parseInt(time_limit) || 1000, memory_limit: parseInt(memory_limit) || 256, created_by: req.session.user.user_id },
